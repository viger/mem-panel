...
// How to generate your own Uid:
// https://www.Uidgenerator.net/
let userID = '89b3cbba-e6ac-485a-9481-976a0415eab9';
let blogPassword = `f@ckgfw!!!`;
...
export default {
     ...
     * @param {{Uid: string, PIP: string, DNS_RESOLVER_URL: string}} env
    ...
    async fetch(request, env) {
        try {          
            ...
            proxyIP = env.PIP || proxyIP;
            dohURL = env.DNS_RESOLVER_URL || dohURL;
            blogPassword = env.BLOG_PWD || blogPassword;
            hashPassword = sha256.sha224(blogPassword);
            if (!isValidUid(userID)) throw new Error(`Invalid Uid: ${userID}`);
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/login':
                        ...
                        if (!secretKey) {
                            ...
                            await env.blog.put('secretKey', secretKey);
                        }

                        if (request.method === 'POST') {
                            const password = await request.text();
                            const savedPass = await env.blog.get('pwd');
                ...
                return url.pathname.startsWith('/tr') 
                    ? await blogOverWSHandler(request) 
...
async function blogOverWSHandler(request) {
    const webSocketPair = new WebSocketPair();
    const [client, webSocket] = Object.values(webSocketPair);
    webSocket.accept();
    let address = "";
 ...
 * Note: This is not a real Uid validation.
 * @param {string} Uid The string to validate as a Uid.
 * @returns {boolean} True if the string is a valid Uid, false otherwise.
 */
...
async function Authenticate (request, env) {
    try {
        const secretKey = await env.blog.get('secretKey');
        const secret = new TextEncoder().encode(secretKey);
...
function buildClashChainOutbound(chainProxyParams) {
    ...
    const { hostName, port, Uid, flow, security, type, sni, fp, alpn, pbk, sid, headerType, host, path, serviceName } = chainProxyParams;
    let chainOutbound = {
...
async function getNormalConfigs(proxySettings, hostName, client) {
    ...
    const trojanPass = encodeURIComponent(blogPassword);
